...
class Trail:
    ...
    walk_in: bool = False
    # The public-land unit the trail runs through - persisted at ingest (``cache._assign_trail_land``,
    # issue #335 PR 2) from a spatial join against ``public_land``, not recomputed per request.
    # ``agency`` is the owner ("USFS", "BLM", ...), ``unit`` the area name ("Six Rivers National
    # Forest"). Both ``None`` when no cached ownership polygon contains the trail. Lets the card
    # say "on Six Rivers NF".
