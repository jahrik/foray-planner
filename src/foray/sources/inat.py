...
# used by foray.sources.inat_bulk.load_inat (applied at load time, so a *future* bulk-load
# doesn't reintroduce the gap fixed by the now-deleted one-time scripts/backfill_obscured.py).
# Only ever a hint that a row is obscured, never proof it's precise - resync's
