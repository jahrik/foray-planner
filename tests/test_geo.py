...
def test_cells_in_radius_covers_the_home_disk() -> None:
    ...
    radius_km = 60.0
    cells = set(cells_in_radius(home_lat, home_lng, radius_km, RES))

    step_deg = 0.1
    for dlat in range(-10, 11):
        for dlng in range(-10, 11):
            lat = home_lat + dlat * step_deg
            lng = home_lng + dlng * step_deg
            if haversine_km(home_lat, home_lng, lat, lng) <= radius_km:
                assert grid_cell(lat, lng, RES).cell_id in cells


def test_cells_in_radius_ids_round_trip_through_grid_cell() -> None:
    # Every id cells_in_radius hands back must be a real H3 cell at RES - decoding its center
    # and re-encoding must land back on the same id.
    for cell_id in cells_in_radius(0.1, 0.1, 40.0, RES):
        clat, clng = grid_cell_center(cell_id)
        assert grid_cell(clat, clng, RES).cell_id == cell_id
...
def test_cells_along_segment_includes_the_line() -> None:
    start = (44.0, -121.0)
    dest = (45.2, -122.4)
    cells = set(cells_along_segment(*start, *dest, 20.0, RES))
