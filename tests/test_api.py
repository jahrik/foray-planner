...
def test_refresh_rate_limits_repeat_triggers_from_same_ip(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("foray.refresh.ingest", lambda *args, **kwargs: None)
...
def test_refresh_concurrent_requests_start_only_one(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("foray.refresh.ingest", fake_ingest)
    monkeypatch.setattr("foray.scoring.build_phenology", lambda *args, **kwargs: None)
...
def test_healthz_data_fire_and_precip_are_non_blocking(client: TestClient, con: psycopg.Connection) -> None:
    ...
    for prefix in ("obs:fungi:place:1:x", "land:coverage", "trails:place:1:q1", "dispersed:place:1:v1"):
